...
def _repair_dhis2_ou_level_query_filters(query_obj: QueryObjectDict) -> bool:
    """Prevent live Explore requests from filtering on ``SUM(ou_level)``.

    Chart-save normalization only sees persisted form data. The QueryObject
    generated by an unsaved Explore request uses ``filter``/``col``, so repair
    it immediately before SQL generation as a final DHIS2 safety guard.
    """
    changed = False
    for filter_key in ("filter", "filters"):
        for filter_item in query_obj.get(filter_key) or []:
...
def _is_dhis2_serving_dataset(dataset: Any) -> bool:
    """Recognize older DHIS2 wrappers while their schema is being repaired."""
...
class SqlaTable(
    ...
    def query(self, query_obj: QueryObjectDict) -> QueryResult:
        ...
        if _is_dhis2_serving_dataset(self) and _repair_dhis2_ou_level_query_filters(
            query_obj
        ):
            logger.info(
                "Repaired aggregate ou_level filter in live DHIS2 query for dataset id=%s",
                self.id,
            )
