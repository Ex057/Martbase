...
export default function DHIS2SyncHistory() {
  ...
  const [typeFilter, setTypeFilter] = useState<'both' | 'sync' | 'metadata'>(
    'both',
  );
  ...
  const [requestLogs, setRequestLogs] = useState<
    Record<
      number,
      {
        logs: DHIS2RequestLog[];
        summary: DHIS2RequestLogSummary | null;
        loading: boolean;
      }
    >
  >({});
  ...
  const callJobAction = useCallback(
    async (
      job: DHIS2AnyJob,
      action: 'cancel' | 'restart' | 'pause' | 'resume',
  ...
  const deleteJob = useCallback(
    async (job: DHIS2AnyJob) => {
      ...
      try {
        ...
        setJobs(prev =>
          prev.filter(
            j => j.id !== job.id || j.job_category !== job.job_category,
          ),
        );
      } catch (error) {
  ...
  const fetchRequestLogs = useCallback(async (jobId: number, quiet = false) => {
    if (!quiet) {
      setRequestLogs(prev => ({
        ...prev,
        [jobId]: { logs: [], summary: null, loading: true },
      }));
    }
    try {
        ...
        {activeCount > 0 && (
          <Alert
            ...
            message={
              <Space>
                <Text strong>
                  {t('%s job(s) currently running', activeCount)}
                </Text>
              ...
              columns={[
                {
                  ...
                  render: (_: unknown, job: DHIS2AnyJob) => (
                    <Tag
                      color={
                        job.job_category === 'metadata' ? 'purple' : 'blue'
                      }
                    >
                      {job.job_category === 'metadata'
                        ? t('Metadata')
                        : t('Sync')}
                    </Tag>
                  ),
                ...
                },
                {
                  ...
                  render: (_: unknown, job: DHIS2AnyJob) => {
                    ...
                    const extracted = isSyncJob(job)
                      ? (job.rows_extracted ?? null)
                      : null;
                    const fetchedDiffers =
                      extracted !== null &&
                      extracted > 0 &&
                      extracted !== loaded;
                    ...
                    if (fetchedDiffers) {
                      return (
                        <Tooltip
                          title={t(
                            '%s fetched from DHIS2, %s written to local staging',
                            String(extracted),
                            String(loaded),
                          )}
                        >
                ...
                },
                {
                  ...
                  render: (_: unknown, job: DHIS2AnyJob) => {
                    ...
                    const restartTooltip = restartBlocked
                      ? t(
                          'Another job for this dataset is currently running. Cancel it first.',
                        )
                      ...
                      <Space size={4} wrap>
                          ...
                          <Tooltip title={t('Cancel this job')}>
                            ...
                            >
                              <Button
                                ...
                                loading={
                                  !!actionLoading[
                                    `${job.job_category}-${job.id}-cancel`
                                  ]
                                }
                                size="small"
                              >
                        ...
                        {isActive && isSyncJob(job) && (
                          <Tooltip title={t('Cancel and deactivate dataset')}>
                            <Popconfirm
                              title={t(
                                'Pause dataset and cancel job #%s?',
                                String(job.id),
                              )}
                        ...
                        {isPaused && isSyncJob(job) && (
                          <Tooltip
                            title={t('Re-activate dataset for scheduled syncs')}
                          >
                            <Button
                              icon={<PlayCircleOutlined />}
                              loading={
                                !!actionLoading[
                                  `${job.job_category}-${job.id}-resume`
                                ]
                              }
                              size="small"
                        ...
                        {isTerminal && (
                          <Tooltip title={restartTooltip}>
                            <Button
                              ...
                              loading={
                                !!actionLoading[
                                  `${job.job_category}-${job.id}-restart`
                                ]
                              }
                              size="small"
                              onClick={
                                restartBlocked
                                  ? undefined
                                  : () => void callJobAction(job, 'restart')
                              }
                            >
                              {t('Restart')}
                            </Button>
                          ...
                          <Tooltip title={t('Delete this job record')}>
                            ...
                            >
                              <Button
                                ...
                                loading={
                                  !!actionLoading[
                                    `${job.job_category}-${job.id}-delete`
                                  ]
                                }
                                size="small"
                              />
                ...
                expandedRowRender: (job: DHIS2AnyJob) => {
                  ...
                  const renderMetadataInstances = () => {
                    if (instEntries.length === 0) {
                      return (
                        <Text type="secondary">
                          {t('No instance details recorded')}
                        </Text>
                      );
                    }
                    return (
                      ...
                      >
                        {instEntries.map(([instId, typeResults]) => {
                          const typeMap = typeResults as Record<
                            string,
                            DHIS2MetadataTypeResult
                          >;
                          ...
                          const totalCount = typeRows.reduce(
                            (s, r) => s + (r.count || 0),
                            0,
                          );
                          const failedTypes = typeRows.filter(
                            r =>
                              r.status !== 'success' &&
                              r.status !== 'unsupported',
                          );
                          const instanceName =
                            nameMap[instId] || `Instance ${instId}`;
                          ...
                          return (
                            <Card
                              ...
                              style={{
                                borderColor: allOk ? '#b7eb8f' : '#ffccc7',
                              }}
                            >
                              <Table<{
                                typeName: string;
                              ...
                              <Space direction="vertical" size={2}>
                                ...
                                {r.rows !== undefined && (
                                  <Text style={{ fontSize: 12 }}>
                                    {t('Rows loaded')}:{' '}
                                    <Text strong>{String(r.rows)}</Text>
                                  </Text>
                                )}
                                {r.sync_mode && (
                                  <Text
                                  ...
                                  children: (() => {
                                    ...
                                    if (logData.logs.length === 0) {
                                      return (
                                        <Text type="secondary">
                                          {ACTIVE_STATUSES.has(job.status)
                                            ? t(
                                                'No request logs yet — sync is starting…',
                                              )
                                            : t(
                                                'No request logs recorded for this job.',
                                              )}
                                        </Text>
                                      );
                                    }
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        {summary && (
                                          ...
                                          >
                                            ...
                                            >
                                              {t(
                                                '%s rows fetched',
                                                summary.total_rows_fetched.toLocaleString(),
                                              )}
                                            </Tag>
                                            <Tag
                                              bordered={false}
                                        ...
                                        {logData.logs.some(
                                          l => l.dhis2_error_code === 'E7144',
                                        ) && (
                                          <Alert
                                            type="error"
                                            showIcon
                                            message={t(
                                              'Analytics tables not built (E7144)',
                                            )}
                                            description={t(
