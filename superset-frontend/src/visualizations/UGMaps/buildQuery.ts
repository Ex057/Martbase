...
export default function buildQuery(formData: QueryFormData) {
  ...
  const resolveSelectedLevel = (): number | undefined => {
    if (Array.isArray(boundary_levels) && boundary_levels.length > 0) {
      const parsed = boundary_levels
    ...
    if (Array.isArray(boundary_level) && boundary_level.length > 0) {
      const parsed = Number(boundary_level[0]);
      if (Number.isFinite(parsed) && parsed > 0) {
        return parsed;
      }
    }
    if (boundary_level !== undefined && boundary_level !== null) {
      const parsed = Number(boundary_level);
      if (Number.isFinite(parsed) && parsed > 0) {
        return parsed;
      }
    }
  ...
  const simpleBoundaryLevel = Number(formDataAny?.geo_boundary_level);
  const effectiveSelectedLevel =
    Number.isFinite(simpleBoundaryLevel) && simpleBoundaryLevel > 0
      ? simpleBoundaryLevel
      : selectedLevel;
  const selectedLevelColumn =
    effectiveSelectedLevel === 2
      ? region_column || org_unit_column
      : effectiveSelectedLevel === 3
  ...
  const geoJoinKeyColumn =
    formDataAny?.geo_join_key_column || entityColumn
      ? sanitizeDHIS2ColumnName(
          String(formDataAny?.geo_join_key_column || entityColumn),
        )
      : undefined;
  ...
  return buildQueryContext(formData, baseQueryObject => {
    ...
    const isUGMapsViz = String((formData as any)?.viz_type || '') === 'ug_maps';
    ...
    if (!isUGMapsViz && isDHIS2Dataset && !isStagedLocalDataset) {
    ...
    const sanitizedOrgUnitColumn =
      (selectedLevelColumn
        ? sanitizeDHIS2ColumnName(selectedLevelColumn)
        : undefined) || geoJoinKeyColumn;
