...
export default function transformProps(chartProps: ChartProps): DHIS2MapProps {
  ...
  const region_column = formDataAny?.region_column;
  const district_column = formDataAny?.district_column;
  const entityColumn =
    typeof formDataAny?.entity === 'string'
      ? formDataAny.entity
      : formDataAny?.entity?.label || formDataAny?.entity?.column_name;
  const geo_join_key_column = formDataAny?.geo_join_key_column || entityColumn;
  const geo_join_feature_property = formDataAny?.geo_join_feature_property;
  const geo_boundary_level = formDataAny?.geo_boundary_level;
  ...
  if (formDataAny?.viz_type === 'ug_maps') {
  ...
  const requestedGeoBoundaryLevel = normalizeLevels(geo_boundary_level)[0];
  const requestedPrimaryLevel =
    requestedGeoBoundaryLevel ||
    requestedBoundaryLevels[0] ||
    requestedBoundaryLevelFallback[0];
  const selectedLevelColumn =
    requestedPrimaryLevel === 2
      ? region_column || org_unit_column
      : requestedPrimaryLevel === 3
        ? district_column || org_unit_column
        : org_unit_column;
  const sanitizedSelectedLevelColumn = selectedLevelColumn
    ? sanitizeDHIS2ColumnName(selectedLevelColumn)
    : undefined;
  const sanitizedGeoJoinKeyColumn = geo_join_key_column
    ? sanitizeDHIS2ColumnName(geo_join_key_column)
    : undefined;
  ...
  // Priority 1: Use selected level column (try both original and sanitized)
  if (selectedLevelColumn && allColumns.includes(selectedLevelColumn)) {
    hierarchyLevelColumn = selectedLevelColumn;
  } else if (
    sanitizedSelectedLevelColumn &&
    allColumns.includes(sanitizedSelectedLevelColumn)
  ) {
    hierarchyLevelColumn = sanitizedSelectedLevelColumn;
  } else if (org_unit_column && allColumns.includes(org_unit_column)) {
    ...
    sanitizedGeoJoinKeyColumn &&
    allColumns.includes(sanitizedGeoJoinKeyColumn)
  ) {
    hierarchyLevelColumn = sanitizedGeoJoinKeyColumn;
  }
  ...
  const autoBoundaryLevelColumns = datasourceHierarchyLevels.reduce<
  ...
  const boundaryLevelColumns: Record<number, string> = {
    ...autoBoundaryLevelColumns,
  };
  const sanitizedRegionColumn = region_column
    ? sanitizeDHIS2ColumnName(region_column)
    : '';
  const sanitizedDistrictColumn = district_column
    ? sanitizeDHIS2ColumnName(district_column)
    : '';
  if (sanitizedRegionColumn) {
    boundaryLevelColumns[2] = sanitizedRegionColumn;
  }
  if (sanitizedDistrictColumn) {
    boundaryLevelColumns[3] = sanitizedDistrictColumn;
  }
  ...
  if (requestedGeoBoundaryLevel) {
    selectedLevels = [requestedGeoBoundaryLevel];
  }
  ...
  return {
    ...
    districtColumn: district_column,
    geoJoinKeyColumn: geo_join_key_column,
    geoJoinFeatureProperty: geo_join_feature_property || 'NAME_1',
    metric: metricColumn,
