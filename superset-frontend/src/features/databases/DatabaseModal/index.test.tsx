...
describe('DatabaseModal', () => {
  ...
  test('does not advance the staged DHIS2 edit flow when the database update returns a 422-shaped error payload', async () => {
    const onHide = jest.fn();
    const fetchResource = jest.fn().mockResolvedValue({});
    const updateResource = jest.fn().mockResolvedValue({
      error: 'Select repository reporting units before saving.',
      message: {
        repository_org_unit_config: [
    ...
    expect(await screen.findByText(/step 2 of 5/i)).toBeInTheDocument();
    const initialFetchCalls = fetchResource.mock.calls.length;
    ...
    expect(fetchResource).toHaveBeenCalledTimes(initialFetchCalls);
